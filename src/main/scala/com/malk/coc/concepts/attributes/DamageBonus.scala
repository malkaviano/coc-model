...
final case class DamageBonus(
  ...
  override val value: Int = body.strength.value + body.size.value match {
    ...
    case x if x < 85  => -1
    case x if x < 125 => 0
    case x if x < 165 => fourFacedDice.roll
    case x if x < 205 => sixFacedDice.roll
    case x if x < 285 => 1 to 2 map { _ => sixFacedDice.roll } reduce { _ + _ }
    case x if x < 365 => 1 to 3 map { _ => sixFacedDice.roll } reduce { _ + _ }
    case x if x < 445 => 1 to 4 map { _ => sixFacedDice.roll } reduce { _ + _ }
    case x if x < 525 => 1 to 5 map { _ => sixFacedDice.roll } reduce { _ + _ }
    case x =>
      val y = x - 524
      val dices = (y / 80.0).ceil.toInt + 5
      1 to dices map { _ => sixFacedDice.roll } reduce { _ + _ }
  }
}
